...
**Known follow-ups (surfaced by the review, not S7 defects):** (1) the first-3 Received headers shown to the classifier (`received` in build_user_message) are NOT run through `_sanitize_for_delimiter`, while the new origin-hop header IS — a pre-existing prompt-injection surface for the classifier (sibling to C8, the learner's version); awaiting Matt's decision to fix-now or schedule. (2) 6 of the 19 new tests are benign absence-guards (pass on base and HEAD) — regression guards, not feature-proof.

**Audit status after this session:** Sessions 1, 2, 3, 4, 5, 6, 7, 10 complete and committed. Still open in Part 4: Session 8 (reply understanding), 9 (daily report correctness), 11 (learner robustness), 12 (IMAP runtime safety), 13 (build/packaging hygiene), 14 (private eval corpus), 15+ (spam-improvement). Plus the planned safe sender-approval feature, the M18 follow-up, and the new received-sanitization follow-up.

**Next step:** Matt to decide on the received-sanitization follow-up, then choose the next session.
