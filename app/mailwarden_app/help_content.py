...
UNWANTED_CATEGORIES_HELP = (
    ...
    "Choosing accounts: when you have more than one account set up, the add "
    "form lists them with a checkbox each, all ticked by default (the rule "
    "applies everywhere). Untick the ones you want left alone — a rule only "
    "acts on the accounts you leave ticked. Each rule's card shows which "
    "accounts it covers.\n\n"
    ...
    "list or newsletter but doesn't name anything exact to match on, MailWarden "
    "asks whether there's a subject tag it can watch for — type it in for "
    "instant matching, or skip it and let the AI judge each email.\n\n"
    ...
    "off or deleting it takes effect on the next check — including the instant "
    "matches, which are added and removed right alongside the rule."
)
